...
public class NewBST<Key extends Comparable<Key>, Value> {
    ...
    private class Node {
        ...
        public Node(Key key, Value val, int size, int height, int compares) {
            this.key = key;
            this.val = val;
            this.size = size;
            this.height = height;
    ...
    private Node put(Node x, Key key, Value val, int compares) {
        if (x == null) {
            return new Node(key, val, 1, 1, compares);
        }
        int cmp = key.compareTo(x.key);
        if (cmp < 0) {
            x.left = put(x.left, key, val, compares + 1);
        } else if (cmp > 0) {
            x.right = put(x.right, key, val, compares + 1);
        } else {
            x.val = val;
        }
        x.size = size(x.left) + size(x.right) + 1;
        x.height = Math.max(height(x.left), height(x.right)) + 1;
        x.compares = compares(x.left) + compares(x.right);
        return x;
    }

    private int compares(Node x) {
        if (x == null) {
            return 0;
        }
        return x.compares;
...
//        int sum = avgCompares(root, 0);
//        return (double)sum / size() + 1;
        return (double)compares(root) / size() + 1;
    }

    private int avgCompares(Node x, int sum) {
        if (x == null) {
            return 0;
        }
        return sum + avgCompares(x.left, sum + 1) + avgCompares(x.right, sum + 1);
    }

    public static void main(String[] args) {
        NewBST<String, Integer> st = new NewBST<>();
        int val = 0;
        while (!StdIn.isEmpty()) {
